...
pair<int, int> mayorArea(BinTree<char> const& t) {
    if (t.empty()) return {0, 0};
    else {
        pair<int, int> s1, s2;
        s1 = mayorArea(t.left());
        s2 = mayorArea(t.right());
        if (t.root() == 'B') {
            return { max(s1.first, s2.first), 0 };
        }
        else {
            int mejorActual = s1.second + s2.second + 1;
            return { max(mejorActual, max(s1.first, s2.first)), mejorActual};
        }
    }
}
