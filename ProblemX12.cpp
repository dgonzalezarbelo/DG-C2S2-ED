...
template <typename T>
bool solve(BinTree<T> const& t) {
    if (t.empty()) return true;
    if (t.left().empty() && t.right().empty()) return true;
    if (t.left().empty() || t.right().empty()) return false;

    queue<pair<BinTree<T>, BinTree<T>>> pending;
    pending.push({ t.left(), t.right() });
    while (!pending.empty()) {
        auto [iz, der] = pending.front();
        pending.pop();
        if (!iz.left().empty() || !der.right().empty()) {
            if (iz.left().empty() || der.right().empty()) return false;
            pending.push({ iz.left(), der.right() });
        }
        if (!iz.right().empty() || !der.left().empty()) {
            if (iz.right().empty() || der.left().empty()) return false;
            pending.push({ iz.right(), der.left() });
        }
    }
    return true;
}
