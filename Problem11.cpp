...
//El algoritmo es lineal en el tamaño de la lista this, porque en el bucle se avanza uno por uno en los nodos de la lista
//y una vez este termina simplemente se junta lo que queda de la segunda lista al final de this
//O(this->num_elems)
void ListLinkedDouble::zip(ListLinkedDouble& other) {
    Node* current_this = this->head->next;
    Node* current_other = other.head->next;
    Node* other_next;
    while (current_this != this->head && current_other != other.head) {
        other_next = current_other->next;
        current_this->next->prev = current_other;
        current_other->prev = current_this;
        current_other->next = current_this->next;
        current_this->next = current_other;
        current_this = current_other->next;
        current_other = other_next;
    }
    if (current_other != other.head) {
        Node* last = this->head->prev;
        last->next = current_other;
        current_other->prev = last;
