...
template <typename T> void solve(BinTree<T> const& t) {
    bool BBA = true;
    T inf;
    bool initialized = false;
    t.inorder([&BBA, &inf, &initialized](T x) {
        if (!initialized) {
            inf = x;
            initialized = true;
            return true;
...
bool resuelveCaso() {
    ...
    if (tipo == 'N') solve(read_tree<int>(std::cin));
    else solve(read_tree<std::string>(std::cin));
    return true;
}
