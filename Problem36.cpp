...
private:
    ...
    std::pair<bool, T> lower_bound_aux(T const& elem, Node* root) const {
        if (root == nullptr) return { false, 0 };
        else {
            if (root->elem == elem) return { true, elem };
            else {
                if (elem < root->elem) {
                    auto sol = lower_bound_aux(elem, root->left);
                    if (sol.first) return sol;
                    else return { true, root->elem };
                }
                else return lower_bound_aux(elem, root->right);
            }
        }
    }
...
std::pair<bool, T> SetTree<T>::lower_bound(T const& elem) const {
    return lower_bound_aux(elem, this->root_node);
}
