...
pair<int, int> solve(BinTree<int> const& t) {   //{Tramos navegables, afluente}
    if (t.empty()) return { 0, 0 };
    if (t.left().empty() && t.right().empty()) return { 0, 1 };
    auto i = solve(t.left());
    auto d = solve(t.right());
    int navegables = i.first + d.first + (i.second >= 3 ? 1 : 0) + (d.second >= 3 ? 1 : 0);
    int afluente = i.second + d.second - t.root();
    if (afluente < 0) afluente = 0;
    return { navegables, afluente };
}

void resuelveCaso() {
    auto t = leer_arbol<int>(-1);
    auto sol = solve(t);
    printf("%d\n", sol.first);
}
