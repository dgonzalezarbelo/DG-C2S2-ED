...
private:
    ...
    template <typename U> class gen_reverse_iterator;
...
public:
    ...
    using reverse_iterator = gen_reverse_iterator<T>;
    using reverse_const_iterator = gen_reverse_iterator<T>;

    iterator begin() { return iterator(head, head->next); }

    reverse_iterator rBegin() { return reverse_iterator(head, head->prev); }

    iterator end() { return iterator(head, head); }

    reverse_iterator rEnd() { return reverse_iterator(head, head); }

    const_iterator begin() const { return const_iterator(head, head->next); }

    reverse_iterator rBegin()const  { return reverse_iterator(head, head); }

    const_iterator end() const { return const_iterator(head, head); }

    reverse_iterator rEnd() const { return reverse_iterator(head, head->prev); }
...
private:
    ...
    public:
        ...
        bool operator==(const gen_reverse_iterator& other) const {
            return (head == other.head) && (current == other.current);
        }
    ...
    private:
        gen_reverse_iterator(Node* head, Node* current) : head(head), current(current) {}
        Node* head;
        Node* current;
    };
...
void resuelveCaso() {
    ...
    if (lista.empty()) {
        ...
        printf("%d", lista.rBegin().elem());
        for (auto it = lista.rBegin().advance(); it != lista.rEnd(); it.advance()) {
            printf("% d", it.elem());
        }
        printf("\n");
    }
