...
BinTree<int> reconstruirAux(vector<int> const& preorden, vector<int> const& inorden, int& i, int& j, int root) {
    if (j < inorden.size()) {
        if (inorden[j] == root) return BinTree<int>();
        else if (i < preorden.size()) {
            if (preorden[i] == inorden[j]) {
                int raiz = preorden[i];
                BinTree<int> right = reconstruirAux(preorden, inorden, ++i, ++j, root);
                return BinTree<int>({}, raiz, right);
            }
            else {
                int raiz = preorden[i];
                BinTree<int> left = reconstruirAux(preorden, inorden, ++i, j, raiz);
                BinTree<int> right = reconstruirAux(preorden, inorden, i, ++j, root);
                return BinTree<int>(left, raiz, right);
            }
        }
...
BinTree<int> reconstruir(vector<int> const& preorden, vector<int> const& inorden) {
    int i = 0, j = 0;
    return reconstruirAux(preorden, inorden, i, j, -1);
}
...
bool resuelveCaso() {
    ...
    cin >> num;
    if (!cin) return false;
    vector<int> preorden(num), inorden(num);
    for (int& x : preorden) cin >> x;
    for (int& x : inorden) cin >> x;
