...
tuple<int, bool, bool> solve(BinTree<char> const& t, int profundidad) {
    if (t.empty()) return { profundidad, true, true };
    else {
        auto s1 = solve(t.left(), profundidad + 1);
        auto s2 = solve(t.right(), profundidad + 1);
        if (get<1>(s1) && get<1>(s2)) {
            if (get<0>(s1) == get<0>(s2) + 1) return { get<0>(s2), false, true };
            else if (get<0>(s1) == get<0>(s2)) return { get<0>(s2), true, true };
            else return { 0, false, false };
        }
        else if (get<1>(s1) && get<2>(s2)) {
            if (get<0>(s1) == get<0>(s2) + 1) return { get<0>(s2), false, true };
            else return { 0, false, false };
        }
        else if (get<2>(s1) && get<1>(s2)) {
            if (get<0>(s1) == get<0>(s2)) return { get<0>(s2), false, true };
            else return { 0, false, false };
        }
        else return { 0, false, false };
    }
...
void resuelveCaso() {
    ...
    auto sol = solve(t, 1);
    if (get<1>(sol)) cout << "COMPLETO\n";
    else if (get<2>(sol)) cout << "SEMICOMPLETO\n";
    else cout << "NADA\n";
}
