 ...
 */
  ...
  */
   ...
   *
   * Estudiante 1: Daniel Gonzalez Arbelo
   * Estudiante 2:
   * Estudiante 3:
   *
   */
...
private:
    ...
    Node* reverse_priv(Node* current, Node*& head, Node*& end);
};
...
ListLinkedSingle::Node* ListLinkedSingle::reverse_priv(Node* current, Node*& head, Node*& end) {
    if (current->next == end) {
        head = current;
        return current;
    }
    else {
        reverse_priv(current->next, head, end)->next = current;
        return current;
    }
}
...
void ListLinkedSingle::reverse_segment(int index, int length) {
    if (length > 1) {
        Node* virt_head = this->nth_node(index);
        Node* virt_end = this->nth_node(index + length);
        reverse_priv(virt_head, virt_head, virt_end)->next = virt_end;
        if (index != 0)
            this->nth_node(index - 1)->next = virt_head;
        else
            this->head = virt_head;
    }
}
